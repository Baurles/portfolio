...
export const SpiningText = () => {
  return(
    ...
    whileHover={logoHover}>
      <span className="sr-only" style={{display:"none"}}>{text}</span>
      <motion.span 
      initial="hidden"
      animate="visible" 
      ...
      aria-hidden>
        {text.split('').map((char,index)=>(
         <motion.span id="target" key={index} style={{display:"inline-block"}} variants={defaultAnimationsLetters}>{char === " " ? "\u00A0": char}</motion.span>
    ))}
    </motion.span>
    ...
    <motion.span
     ...
     animate={defaultAnimationsPointer.animate}>|</motion.span>
   </motion.h1>
